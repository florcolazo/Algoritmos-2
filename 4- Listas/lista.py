def __criterio(dato, criterio):
    if(criterio is None):
                # Si no se proporciona un criterio, devolver el dato sin cambios

        return dato
    else:
        # Si se proporciona un criterio, devolver el valor correspondiente al criterio en el dato
...
class Lista(object):
    ...
    def __init__(self):
        # Método constructor, inicializa cualquier atributo necesario
        self.__elementos = []
    ...
    def eliminar(self, dato, criterio=None, clave=None, criterio_clave=None):
        # Buscamos la posición del dato a eliminar
        pos = self.busqueda(dato, criterio, clave, criterio_clave)
         # Si encontramos el dato, lo eliminamos y lo retornamos
        if(pos != -1):
